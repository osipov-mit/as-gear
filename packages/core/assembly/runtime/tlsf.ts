...
//@ts-ignore: decorator
@external("env", "alloc")
export declare function alloc(r: i32): i32;
...
function growMemory(root: Root, size: usize): void {
  ...
  if (alloc(pagesWanted) < 0) {
    if (alloc(pagesNeeded) < 0) unreachable();
  }
...
function initialize(): void {
  ...
  if (pagesNeeded > pagesBefore && alloc(pagesNeeded - pagesBefore) < 0) unreachable();
